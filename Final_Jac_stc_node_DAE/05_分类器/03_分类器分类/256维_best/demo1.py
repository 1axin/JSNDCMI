...
#***************************划分训练集************************************
SampleFeature = []
ReadMyCsv(SampleFeature, 'SampleFeature_DAE_分类器_256.csv')
...
for train, test in cv.split(SampleFeature, SampleLabel):
    model = GradientBoostingClassifier(n_estimators=30, max_depth=5)
    ...
    # plt.plot(recall, precision, lw = 2, label= ' (area = %0.2f)' % average_precision_score(SampleLabel[test], predicted[:, 1]))
